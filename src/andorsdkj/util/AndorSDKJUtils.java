...
public class AndorSDKJUtils {
    ...
    public static boolean getBool(AndorCamera pAndorCamera,
                                  String pKey
    ) throws AndorSdkJException {
        if (mDebugMessages)
            System.out.println("Getting a boolean feature: " + pKey + " for the camera: " + pAndorCamera);

        Pointer<Character> lKeyPointer = Pointer.pointerToWideCString(pKey);
        Pointer<Integer> lValuePointer = Pointer.allocateInt();
        handleErrorWithException("Cannot get boolean value for " + pKey,
                AtcoreLibrary.AT_GetBool(pAndorCamera.getHandle(),
                        lKeyPointer,
                        lValuePointer));
